...
export default function ProductTable({ products, onEdit, onDelete, deletingId, onRequestDelete }: ProductTableProps) {
  if (products.length === 0) {
    return (
      <div className="text-center py-16 text-text-muted">
        <span aria-hidden="true" className="material-symbols-outlined text-5xl mb-4 block">inventory_2</span>
              ...
              <td className="py-3 px-3 sm:px-4 text-right">
                <div className="flex items-center justify-end gap-1">
                  ...
                  {deletingId === product.id ? (
                    <>
                      <button
                        onClick={() => { onDelete(product.id); onRequestDelete(null); }}
